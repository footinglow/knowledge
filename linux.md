...
# 記憶階層
- 階層型キャッシュメモリ
  - もっともレジスタに近いのがL1キャッシュ、L2、L3とある場合もある
  - /sys/devices/system/cpu/cpu0/cache/index0
- Simultaneous Multi Threading(SMT)
  - CPUはメモリアクセス待ちなどの待ち時間が発生する。
  - CPUの中にレジスタなどを2セット分用意し、2コアにみせて、一つのコアがメモリアクセス待ちなどでCPUが秋になる場合、もう一つのコアが処理をする
  - プロセス、スレッドは それぞれのコアに割り当てられる。うまく待ち時間がずれると高速化の効果が大きいが、2コアともメモリアクセスばかりしていると高速化の効果は得られない。
- ページキャッシュ
  - ファイル書き込み時に、メモリに書いた時点でreturnする。ファイルまで書き込みたい場合は、open時に「O_SYNC」を設定する
  - direct I/Oを使用するとページキャッシュを使わずファイルを読み書きするが、確実に書き込みを保証するならO_SYNCが必須
  - sar -r
- スワップ
  - スラッシング＝メモリアクセスのたびに、ページイン、ページアウトが発生し、ストレージのアクセスが発生すること
  - sar -B
  - swap --show : スワップ領域表示
  - sar -W 1 : 1秒当たりのスワップ数
  - sar -S   : スワップ領域使用量
- 仮想化機能
  - KVM(Kernel-based Virtual Machine):Linuxカーネルが提供する仮想化環境
  - QEMU:CPU及びハードウェアのエミュレータ。KVMと組み合わせて使う場合はCPUのエミュレーション部分は使わない
  - virt-manager:仮想マシンの生成・管理・廃棄をする実行はQEMU
